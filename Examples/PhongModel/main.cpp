...
class ScenePhong : public IScene
{
public:
    ...
    ScenePhong() :  mMVP(), mModelView(), mLocations(), mKa(), mKd(), mKs(), mShininess(),
                    mLa(), mLd(), mLs(), mLp() {}
    ...
    void Initialize(const std::shared_ptr<Camera>& camera) override
    {
        ...
        auto VSSC = Dazzle::FileManager::ReadFile("shaders\\Phong.vs.glsl");
        auto FSSC = Dazzle::FileManager::ReadFile("shaders\\Phong.fs.glsl");
...
public:
    ...
    void SetScene(IScene* scene) override { mScene = static_cast<ScenePhong*>(scene); }
    ...
    void Update() override
    {
        ...
        // Shader
        ImGui::SeparatorText("Phong Reflection Model");
