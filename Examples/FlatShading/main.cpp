...
public:
    ...
    SceneFlat() :   mMVP(), mModelView(), mLocations(), mKa(), mKd(), mKs(), mShininess(),
                    mLa(), mLd(), mLs(), mLp() {}
    ...
    void Initialize(const std::shared_ptr<Camera>& camera) override
    {
        ...
        mSphere = std::make_unique<Dazzle::Sphere>(1.0f, 32, 32, 0.0f, glm::pi<float>(), 0.0f, glm::two_pi<float>()); 
        mSphere->InitializeBuffers();

        // -----------------------------------------------------------------------------------------
        // Shader Program
        // Get the source code for shaders
        auto VSSC = Dazzle::FileManager::ReadFile("shaders\\FlatShader.vs.glsl");
        auto FSSC = Dazzle::FileManager::ReadFile("shaders\\FlatShader.fs.glsl");
    ...
    void SetProvokingVertexConvention(GLenum convention) { glProvokingVertex(convention); }
...
class UIFlat : public IUserInterface
{
public:

    void SetScene(IScene* scene) override { mScene = static_cast<SceneFlat*>(scene); }
    void SetCamera(Camera* camera) override { mCamera = camera; }

    void Update() override
    {
        static int convention = 1;
        static int lastConvention = 1;
        ...
        // Shader
        ImGui::SeparatorText("Flat Shading");

        // Provoking Vertex Convention
        // Specify the vertex to be used as the source of data for flat shaded varyings
        ImGui::RadioButton("GL_FIRST_VERTEX_CONVENTION", &convention, 0);
        ImGui::RadioButton("GL_LAST_VERTEX_CONVENTION", &convention, 1);
        ImGui::NewLine();
        if (convention == 0) 
            mProvokingVertexConvention = GL_FIRST_VERTEX_CONVENTION;
        else
            mProvokingVertexConvention = GL_LAST_VERTEX_CONVENTION;
        ...
        if (mScene)
        {
            ...
            if (lastConvention != convention)
            {
                mScene->SetProvokingVertexConvention(mProvokingVertexConvention);
...
private:
    ...
    SceneFlat* mScene = nullptr;
    Camera* mCamera = nullptr;

    GLenum mProvokingVertexConvention = GL_LAST_VERTEX_CONVENTION;
...
int main(int argc, char const *argv[])
{
    ...
    config.title = "Flat Shading";  // Window Title

    auto sceneFlat = std::make_unique<SceneFlat>();
    auto uiFlat = std::make_unique<UIFlat>();

    App app(config, std::move(sceneFlat), std::move(uiFlat));
    app.Run();
